...
class MPC_Controller:
    def __init__(self): 
        self.TAU = 1
        self.FILTER1 = 0.275/0.015 * 0.82 # demping for delta
        self.FILTER2 = 1/0.015 * 1.1 # udempet svingefrekvens for andre ordens filter delta
        self.MASS = 375
        self.L = 1.8
        self.ENGINEBREAKS = 150
        self.THROTTLEGAIN = 25
        ...
        #R matrix
        U_1_WEIGHT = 0.001  #Throttle punishment
        U_2_WEIGHT = 50     #steering

        #Input bounds
        THROTTLE_LOW = 0      
        THROTTLE_HIGH = 100
        STEERING_LOW = -pi/180 * 25  
        STEERING_HIGH = pi/180 * 25 
        ...
        DELTA_LOW = -pi/180 * 25         
        DELTA_HIGH = pi/180 * 25

        N_STEPS = 500   # hvis dette endres må N_STEPS i simulink endres også
        ...
        F_dot = 1/self.TAU * (-F + u_1 * self.THROTTLEGAIN)
        delta_d = delta_dot
        delta_ddot = -self.FILTER1 * delta_dot - self.FILTER2 * (delta - u_2) 
        ...
        for i in range(N_STEPS):
            t = i * 0.1
            t = t * (2*pi) / (N_STEPS * 0.1)
            a = 30
            self.xref_list.append(a * sin(t))
            self.yref_list.append(a/2 * sin(2*t))
            self.vref_list.append(0)
            #self.xref_list.append(50 * sin(t))
            #self.yref_list.append(50 * cos(t))
            #self.vref_list.append(0)
        ...
        def tvp_fun(t_now):
            ...
            for k in range(self.PREDICTION_HORIZON):
                ...
                ref_index = (step + k) % len(self.xref_list)
                ...
                tvp_template['_tvp', k, 'velocity_ref'] = self.vref_list[ref_index]
    ...
    def state_estimator(self, physical_states):
        ...
        self.F_state += dt *( 1/self.TAU * (-self.F_state + self.u_1_state * self.THROTTLEGAIN))
        self.delta_state += dt*(self.delta_dot_state)
        self.delta_dot_state += dt*(-self.FILTER1 * self.delta_dot_state - self.FILTER2 * (self.delta_state - self.u_2_state))
        self.previous_time = time.time()
